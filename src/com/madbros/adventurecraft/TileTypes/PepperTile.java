	...
	public void deleteMe(int x, int y, Block[][] activeBlocks) {
		...
		if(currentSpriteId == 3) {
			for(int i = dropAmount1; i >-1 ; i--) {
				...
				Game.collectibleController.add(PEPPER_SPROUT, Sprites.sprites.get(Sprites.PEPPER_SPROUT), collectibleRect, 1);
			}
			for(int i = dropAmount2; i >-1 ; i--) {
				Rect collectibleRect = new Rect(activeBlocks[x][y].absRect.x, activeBlocks[x][y].absRect.y, 32, 32);
				Game.collectibleController.add(PEPPER, Sprites.sprites.get(Sprites.PEPPER), collectibleRect, 1);
			}
		} else {
			Rect collectibleRect = new Rect(activeBlocks[x][y].absRect.x, activeBlocks[x][y].absRect.y, 32, 32);
			Game.collectibleController.add(PEPPER_SPROUT, Sprites.sprites.get(Sprites.PEPPER_SPROUT), collectibleRect, 1);
